...
class Crashlytics {
  ...
  async recordError(error, jsErrorName, attributes = {}) {
    try {
      ...
      await firebaseRecordError(getCrashlytics(), error, jsErrorName);
    } catch (error) {
      if (isDevEnv()) {
        console.error(error.message);
      }
  ...
  log(message, attributes = {}) {
    try {
      ...
      firebaseLog(getCrashlytics(), JSON.stringify(message));

      const funcToExecute = {
        userId: id => firebaseSetUserId(getCrashlytics(), JSON.stringify(id)),
